...
if __name__ == '__main__':
    ...
    import P07_3549_labirinto as maze_solver
    ...
    while True: 
        resposta = input("Deseja gerar um labirinto? (s/n): ").strip().lower()
        if resposta != 's':
            break
        ...
        print('Maze 1')
        print_maze(maze)
        resposta = input("Vamos resolver? (s/n): ").strip().lower()
        if resposta != 's':
            break
        caminho = maze_solver.bfs(maze, wall, cheese)
        tamanho = len(caminho)
        print('Caminho encontrado:', caminho)
        print('Tamanho do caminho:', tamanho)
        print("Labirinto com caminho marcado:")
        maze_solver.print_maze_with_path(maze, caminho)
